...
const benefitLabel=x=>title(x?.benefit||x?.type||x?.purpose||"Travel benefit");
...
function outcomeCopy(kind,d){
  if(kind==="fly")return d?.primaryAirline?("Build around "+label(d.primaryAirline)+(d?.status?.stoppingPoint?" with "+d.status.stoppingPoint+" as the stopping point.":".")):"No airline concentration is being forced.";
  if(kind==="airport"){
    const c=d?.capabilities||{},n=Object.values(c).filter(v=>Number(v)>0).length;
    return n?("Your recommended structure preserves "+n+" meaningful airport "+(n===1?"capability.":"capabilities.")):"No incremental airport capability is being claimed.";
  }
  if(kind==="travel"){
    const v=Number(d?.annualTravelValue||d?.value||0);
    return v>0?("The same spending produces about "+money(v)+" of modeled annual travel capacity."):"Use the selected rewards ecosystem to turn existing spend into more usable travel.";
  }
  if(kind==="hotel")return d?.primaryHotel?("Concentrate where it helps: "+label(d.primaryHotel)+(d?.status?.selectedTarget?" with "+d.status.selectedTarget+" as the selected target.":".")):"No hotel-status strategy is being forced.";
  return "";
}
function actionDetail(a){
  const bits=[];
  if(a.annualAmount)bits.push(money(a.annualAmount)+" annually");
  if(a.annualSpendRequired)bits.push(money(a.annualSpendRequired)+" annual threshold");
...
function render(data,{demo=false}={}){
  ...
  '<div class="app">'+
  '<aside class="rail"><div class="brand">Quiet Premium<small>Same spend. A brighter tomorrow.</small></div><nav class="nav">'+
  ...
  '<main class="main"><header class="topbar"><button class="mobile-menu" aria-label="Open navigation">☰</button><div class="wordmark">Quiet Premium</div><div class="meta">Your Personalized Travel Strategy<br><span>Engine '+esc(c.meta?.engineVersion||"—")+' · '+esc(c.meta?.schema||"—")+'</span></div></header>'+
  '<section class="hero" id="overview" data-section="overview"><div class="hero-inner"><div class="eyebrow">Your personalized travel strategy</div><h1>Your Travel Life Gets a Real Upgrade</h1><p>You are already spending the money. This plan shows what changes, what stays the same, and how to turn that same spend into a materially better travel life.</p></div><div class="hero-quote">“Same spend.<br>A brighter tomorrow.”</div></section>'+
  '<div class="content">'+
  '<section class="section" id="travel-life" data-section="travel-life"><div class="section-head"><div><h2>Your New Travel Life</h2><p>Outcomes first. Cards, status and points are the mechanisms underneath.</p></div></div><div class="life-grid">'+
  lifeCard("/band-air.jpeg","✈","Fly Considerably Better",outcomeCopy("fly",tl.flyBetter))+
  lifeCard("/band-lounge.jpeg","⌁","A Smoother Airport Experience",outcomeCopy("airport",tl.airportExperience))+
  lifeCard("/assets/qp-outcome-v210.jpg","☀","More Travel From the Same Spending",outcomeCopy("travel",tl.travelCapacity))+
  ...
  '</div></section>'+
  '<section class="section" id="spend" data-section="spend"><div class="section-head"><div><h2>Your Spending Plan</h2><p>Exact ongoing routing after strategy, status and card architecture are settled.</p></div></div><div class="panel spend-table">'+
