...
function qpCleanMeta(meta={}){
  const allow=new Set(['placement','section','source','page','build','step','data_quality','already_optimized','gap_band','misrouted_band','correction_index','correction_type']);
  const out={};
  Object.entries(meta||{}).forEach(([k,v])=>{if(allow.has(k)&&(typeof v==='string'||typeof v==='number'||typeof v==='boolean'))out[k]=v;});
  return out;
}
...
function track(name,meta={}){
  const clean=qpCleanMeta(meta);
  try{if(typeof gtag==='function')gtag('event',name,clean);}catch(_){}
  qpCentralTrack(name,clean);
}
