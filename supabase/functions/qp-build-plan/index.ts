...
function attach(profile:any,snapshot:any){return{...(profile||{}),verifiedFacts:snapshot}}
function safePlanResult(result:any){
  return{
    status:"ready",
...
Deno.serve(async(req:Request)=>{
  ...
  if(!E||typeof E.analyze!=="function")return json({error:"engine_unavailable"},500,origin);
  ...
  try{
    const normalized=E.normalizeProfile(rawProfile);
