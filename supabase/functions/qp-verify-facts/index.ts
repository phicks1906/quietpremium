...
function fee(t:string){const intro=first(t,[/\$\s*0[^.]{0,80}(?:intro|introductory) annual fee[^.]{0,120}then\s+\$\s*([\d,]+)/i,/(?:intro|introductory) annual fee[^.]{0,100}\$\s*0[^.]{0,120}then\s+\$\s*([\d,]+)/i]);if(intro)return amount(intro[1]);if(/No Annual Fee/i.test(t))return 0;const m=first(t,[/Annual Fee\s*:?\s*\$\s*([\d,]+)/i,/\$\s*([\d,]+)\s+annual fee/i,/annual fee[^$]{0,80}\$\s*([\d,]+)/i,/then\s+\$\s*([\d,]+)[^.]{0,80}annual fee/i]);return m?amount(m[1]):null}
...
function bookingEarn(t:string,id:string){const o:any={};if(id.startsWith("amex_")){const air=rate(t,[/Flights[^.]{0,120}(?:AmexTravel|directly through airlines)/i,/AmexTravel[^.]{0,120}Flights/i]),hotel=rate(t,[/Prepaid Hotels/i]);if(air)o.airfare={direct_airline:air,amex_travel:air};if(hotel)o.hotel={amex_prepaid:hotel}}else if(id==="chase_freedom_unlimited"||id==="chase_freedom_flex"){if(/5%[^.]{0,160}Chase Travel|Chase Travel[^.]{0,160}5%/i.test(t)){o.airfare={chase_travel:5};o.hotel={chase_travel:5}}}else if(id.startsWith("chase_")){const v=rate(t,[/Chase Travel/i]);if(v){o.airfare={chase_travel:v};o.hotel={chase_travel:v}}}else if(id==="united_quest"||id==="united_club"){const renowned=rate(t,[/hotel stays[^.]{0,140}prepay directly through Renowned Hotels and Resorts/i,/hotel accommodation purchases[^.]{0,180}Renowned Hotels and Resorts/i]);if(renowned)o.hotel={renowned_prepaid:renowned}}else if(id==="venture_one"||id==="venture"||id==="venture_x"){const hotel=rate(t,[/hotels[^.]{0,120}Capital One Travel/i,/Capital One Travel[^.]{0,120}hotels/i]),air=rate(t,[/flights[^.]{0,120}Capital One Travel/i,/Capital One Travel[^.]{0,120}flights/i]);if(hotel)o.hotel={capital_one_travel:hotel};if(air)o.airfare={capital_one_travel:air}}return o}
function multiYearCredits(t:string){const o:any={};let m=first(t,[/(?:Global Entry|TSA PreCheck|NEXUS)[^.]{0,280}(?:up to )?\$\s*([\d,]+)[^.]{0,260}every\s+(\d+)\s+years?/i,/\$\s*([\d,]+)[^.]{0,260}(?:Global Entry|TSA PreCheck|NEXUS)[^.]{0,260}every\s+(\d+)\s+years?/i,/statement credit[^.]{0,80}(?:up to )?\$\s*([\d,]+)[^.]{0,220}every\s+(\d+)\s+years?[^.]{0,120}(?:Global Entry|TSA PreCheck|NEXUS)/i]);if(!m){const block=first(t,[/(?:Global Entry|TSA PreCheck|NEXUS)[\s\S]{0,450}every four years/i,/every four years[\s\S]{0,450}(?:Global Entry|TSA PreCheck|NEXUS)/i]);if(block){const a=block[0].match(/\$\s*([\d,]+)/);if(a)o.trusted_traveler={amount:amount(a[1]),years:4};return o}}if(m)o.trusted_traveler={amount:amount(m[1]),years:amount(m[2])};return o}
...
function transferAccess(t:string,id:string){if(!id.startsWith("chase_"))return{};const o:any={};if(/Combine points with other Chase cards with Ultimate Rewards/i.test(t)||/move your points[^.]{0,180}another Chase card with Ultimate Rewards/i.test(t))o.canPool=true;const eligibility=/to transfer points to a travel partner[^.]{0,260}need to have[^.]{0,420}Sapphire Preferred[^.]{0,420}Sapphire Reserve[^.]{0,420}Ink Business Preferred/i.test(t)||/Sapphire Reserve[^.]{0,500}Sapphire Preferred[^.]{0,500}Ink Business Preferred[^.]{0,260}ability to transfer points to travel partners/i.test(t);if(["chase_freedom_unlimited","chase_freedom_flex","chase_freedom_rise"].includes(id)&&eligibility)o.directPartnerTransfer=false;else if((id==="chase_preferred"||id==="chase_reserve")&&(eligibility||/Transfer points to frequent travel programs/i.test(t)))o.directPartnerTransfer=true;if(["chase_freedom_unlimited","chase_freedom_flex","chase_freedom_rise"].includes(id)&&/Combine points with other Chase cards with Ultimate Rewards|move your points[^.]{0,180}another Chase card with Ultimate Rewards/i.test(t))o.canPool=true;return o}
...
function cardRecurringCredits(t:string,id:string){
  ...
  if(id==="amex_platinum"){
    ...
    put("saks_credit",[/\$\s*([\d,]+)\s+Saks(?: Fifth Avenue)? Credit/i]);
...
function airlineTierBenefits(id:string,t:string){
  ...
  if(id==="delta"){
    ...
    const defs:[string,number,RegExp][]=[
      ["Gold",24,/24\s+hours before departure for AAdvantage Gold/i],
      ["Platinum",48,/48\s+hours before departure for AAdvantage Platinum/i],
      ["Platinum Pro",72,/72\s+hours before departure for AAdvantage Platinum Pro/i],
      ["Executive Platinum",100,/100\s+hours before departure for AAdvantage Executive Platinum/i]
    ];
    for(const[tier,h,re]of defs)if(re.test(t))put(tier,{upgradeWindowHours:h});
...
if(kind==="airlines"&&id==="delta"){
  ...
  const defs:[string,RegExp[]][]=[
    ["Silver Medallion",[/\$?\s*([\d,]+)\s+for Silver/i,/Silver[^$0-9]{0,60}\$?\s*([\d,]+)\s*MQD/i]],
    ["Gold Medallion",[/\$?\s*([\d,]+)\s+for Gold/i,/Gold[^$0-9]{0,60}\$?\s*([\d,]+)\s*MQD/i]],
    ["Platinum Medallion",[/\$?\s*([\d,]+)\s+for Platinum/i,/Platinum[^$0-9]{0,60}\$?\s*([\d,]+)\s*MQD/i]],
    ["Diamond Medallion",[/\$?\s*([\d,]+)\s+for Diamond/i,/Diamond[^$0-9]{0,60}\$?\s*([\d,]+)\s*MQD/i]]
  ];
  for(const[tier,res]of defs){const m=first(t,res);if(m)f.thresholds.push({tier,amount:amount(m[1])})}
  if(f.thresholds.length<4){const anchor=t.search(/MQD Thresholds for Status/i),slice=anchor>=0?t.slice(anchor,anchor+1600):t,vals=[...slice.matchAll(/\$\s*([\d,]+)\s*MQDs?/gi)].map(x=>amount(x[1])).filter((x:any)=>x!=null);if(vals.length>=4)f.thresholds=[["Silver Medallion",vals[0]],["Gold Medallion",vals[1]],["Platinum Medallion",vals[2]],["Diamond Medallion",vals[3]]].map(([tier,a])=>({tier,amount:a}))}
}else if(kind==="airlines"&&id==="american"){
  f.thresholds=[];
  if(/40,000\s+Loyalty Points/i.test(t))f.thresholds.push({tier:"Gold",amount:40000});
  if(/75,000\s+Loyalty Points/i.test(t))f.thresholds.push({tier:"Platinum",amount:75000});
  if(/125,000\s+Loyalty Points/i.test(t))f.thresholds.push({tier:"Platinum Pro",amount:125000});
  if(/200,000\s+Loyalty Points/i.test(t))f.thresholds.push({tier:"Executive Platinum",amount:200000});
  ...
  f.thresholds=[];
  const defs:[string,RegExp[]][]=[
    ["Premier Silver",[/Silver[^.]{0,220}([\d,]+)\s+PQP\s*\+\s*([\d,]+)\s+PQF[^.]{0,160}or[^.]{0,80}([\d,]+)\s+PQP/i]],
    ["Premier Gold",[/Gold[^.]{0,220}([\d,]+)\s+PQP\s*\+\s*([\d,]+)\s+PQF[^.]{0,160}or[^.]{0,80}([\d,]+)\s+PQP/i]],
...
async function verify(kind:"cards"|"airlines"|"hotels",id:string){const spec=(ENTITY_SOURCES as any)[kind][id],rows=await Promise.all(spec.urls.map(get)),ok=rows.filter((x:any)=>x.ok&&x.text),text=ok.map((x:any)=>x.text).join(" "),ext=kind==="cards"?cardFacts(id,text):{facts:programFacts(kind,id,text),unresolved:[]},q=complete(spec,ext.facts,text,kind,id),status=ok.length===0?"unavailable":q.complete?"verified":"partial";return{verificationStatus:status,complete:q.complete,verifiedAt:new Date().toISOString(),sources:rows.map(({text,...m}:any)=>m),facts:ext.facts,unresolved:uniq([...(ext.unresolved||[]),...q.missing,...q.unmapped])}}
