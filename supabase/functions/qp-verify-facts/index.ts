...
function cardRecurringCredits(t:string,id:string){
  ...
  const o:any=flexSpecific?{}:{...credits(t)};
...
function hyattMilestoneRewards(t:string){
 const inventory=/20\s+(?:qualifying\s+)?nights/i.test(t)&&/35,?000\s+Base Points/i.test(t)&&/30\s+(?:qualifying\s+)?nights/i.test(t)&&/50,?000\s+Base Points/i.test(t)&&/40\s+(?:qualifying\s+)?nights/i.test(t)&&/65,?000\s+Base Points/i.test(t)&&/50\s+(?:qualifying\s+)?nights/i.test(t)&&/80,?000\s+Base Points/i.test(t)&&/60\s+(?:qualifying\s+)?nights/i.test(t)&&/100,?000\s+Base Points/i.test(t)&&(/70[^\n]{0,180}80[^\n]{0,180}90[^\n]{0,260}(?:qualifying\s+)?nights|70\s+(?:qualifying\s+)?nights[\s\S]{0,1200}90\s+(?:qualifying\s+)?nights/i.test(t))&&/100\s+(?:qualifying\s+)?nights/i.test(t)&&(/110[^\n]{0,220}120[^\n]{0,220}130[^\n]{0,220}140[^\n]{0,300}(?:qualifying\s+)?nights|110\s+(?:qualifying\s+)?nights[\s\S]{0,1800}140\s+(?:qualifying\s+)?nights/i.test(t))&&/150\s+(?:qualifying\s+)?nights/i.test(t),benefits=/2K Next Stay Award/i.test(t)&&/Club Access Award/i.test(t)&&/FIND/i.test(t)&&/Category\s*1\s*[-–]\s*4/i.test(t)&&/Guest of Honor/i.test(t)&&/5K/i.test(t)&&/Suite Upgrade Award/i.test(t)&&/Category\s*1\s*[-–]\s*7/i.test(t)&&/My Hyatt Concierge/i.test(t)&&/10K/i.test(t)&&/AAdvantage Gold/i.test(t)&&/AAdvantage Platinum/i.test(t)&&/Miraval/i.test(t)&&/Ultimate Free Night Award/i.test(t),coverageComplete=inventory&&benefits;
...
if(kind==="airlines"&&id==="delta"){
  ...
  if(id==="marriott"){const amb=first(t,[/Ambassador Elite[^\n]{0,180}?(\d+)\+?\s+nights[^\n]{0,180}?\$\s*([\d,]+)[^\n]{0,80}(?:qualifying )?spend/i,/(\d+)\+?\s+nights[^\n]{0,120}\$\s*([\d,]+)[^\n]{0,120}Ambassador Elite/i]);if(amb)f.thresholds.push({tier:"Ambassador Elite",nights:amount(amb[1]),spend:amount(amb[2])});f.tierBenefits=hotelTierBenefits(id,t)}
