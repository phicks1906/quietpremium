...
function rate(t:string,words:RegExp[]){for(const w of words){const q=w.source;const m=first(t,[new RegExp("(\\d+(?:\\.\\d+)?)\\s*[xX][^.]{0,100}"+q,"i"),new RegExp(q+"[^.]{0,100}(\\d+(?:\\.\\d+)?)\\s*[xX]","i")]);if(m)return Number(m[1])}return null}
function earn(t:string){const g=rate(t,[/all other (?:eligible )?purchases/i,/all other purchases/i,/other eligible purchases/i,/other purchases/i,/every purchase/i]);if(!(g&&g>0))return null;const e:any={dining:g,grocery:g,online_grocery:g,gas_ev:g,online_retail:g,vacation_home:g,airfare:g,hotel:g,general:g};const set=(k:string,v:any)=>{if(v)e[k]=v};set("dining",rate(t,[/restaurants?/i,/dining/i]));const gr=rate(t,[/supermarkets?/i,/groceries/i]);if(gr){e.grocery=gr;e.online_grocery=gr}set("online_grocery",rate(t,[/online groceries/i,/online grocery/i]));set("gas_ev",rate(t,[/gas stations?/i,/EV charging/i]));set("online_retail",rate(t,[/online retail/i]));set("vacation_home",rate(t,[/vacation homes?/i,/Airbnb/i,/Vrbo/i]));const travel=rate(t,[/all other travel/i,/travel purchases/i]);if(travel){e.airfare=travel;e.hotel=travel}set("airfare",rate(t,[/flights? booked directly/i,/flights? booked through AmexTravel/i,/Delta purchases?/i,/United purchases?/i,/American Airlines purchases?/i,/Southwest purchases?/i]));set("hotel",rate(t,[/Hyatt hotels?/i,/Hilton portfolio/i,/Marriott Bonvoy hotels?/i,/hotel stays?/i]));return e}
function bookingEarn(t:string,id:string){const o:any={};if(id.startsWith("amex_")){const air=rate(t,[/Flights[^.]{0,120}(?:AmexTravel|directly through airlines)/i,/AmexTravel[^.]{0,120}Flights/i]),hotel=rate(t,[/Prepaid Hotels/i]);if(air)o.airfare={direct_airline:air,amex_travel:air};if(hotel)o.hotel={amex_prepaid:hotel}}else if(id.startsWith("chase_")){const v=rate(t,[/Chase Travel/i]);if(v){o.airfare={chase_travel:v};o.hotel={chase_travel:v}}}else if(id==="venture"||id==="venture_x"){const hotel=rate(t,[/hotels[^.]{0,120}Capital One Travel/i,/Capital One Travel[^.]{0,120}hotels/i]),air=rate(t,[/flights[^.]{0,120}Capital One Travel/i,/Capital One Travel[^.]{0,120}flights/i]);if(hotel)o.hotel={capital_one_travel:hotel};if(air)o.airfare={capital_one_travel:air}}return o}
...
function cardRecurringCredits(t:string,id:string){
  ...
  }else if(id.startsWith("united_")){Object.assign(o,unitedRecurringCredits(t,id));
  }else if(id==="delta_platinum"||id==="delta_reserve"){
...
function flexibleCaps(t:string,id:string){
  const base=goldCaps(t,id);if(id!=="amex_platinum")return base;
  const caps:any={};const m=first(t,[/up to \$\s*([\d,]+)\s+on these purchases per calendar year/i,/flights?[^.]{0,160}up to \$\s*([\d,]+)[^.]{0,80}per calendar year/i]);if(m)caps.airfare=amount(m[1]);
  return{caps,capGroups:{},groupCaps:{},postCapEarn:{airfare:1}};
}
